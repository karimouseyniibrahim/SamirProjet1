...
@section('content')
    @component(layoutForm() , ['title' => trans('user.user')  , 'model' => 'user' , 'action' => isset($item) ? trans('curd.edit') : trans('curd.add') ])
        ...
        <form action="{{ concatenateLangToUrl('admin/user/item') }}{{ isset($item) ? '/'.$item->id : '' }}"
            ...
            <div class="form-group {{  $errors->has("name")   ? "has-error" : "" }}">
                <div class="form-line">
                    ...
                    <input type="text" name="name" id="name" placeholder="{{ trans('user.name') }}" class="form-control"
            ...
            <div class="form-group {{  $errors->has("email")   ? "has-error" : "" }}">
                <div class="form-line">
                    ...
                    <input type="email" name="email" id="email"
                           {{ isset($item) ? '' : 'required' }} placeholder="{{ trans('user.email') }}"
                           class="form-control" value="{{ isset($item) ? $item->email : old('email')  }}"/>
                    @if ($errors->has("email"))
                        <div class="alert alert-danger">
                        <span class='help-block'>
            ...
            <div class="form-group {{  $errors->has("password")   ? "has-error" : "" }}">
                <div class="form-line">
                    ...
                    <input type="password" name="password" id="password" placeholder="{{ trans('user.password') }}"
                           class="form-control"/>
            ...
            <div class="form-group">
                <div class="form-line">
                    <label for="">{{ trans('user.image') }}</label>
                    @if(isset($item) && $item->image != '')
                        <div class="row">
                            <div class="file-preview-frame krajee-default  kv-preview-thumb" data-fileindex="0"
                                 data-template="image">
                                <div class="kv-file-content">
                                    <img class="img-responsive" style="height: 150px;" alt=""
                                         src="{{ url('/'.env('UPLOAD_PATH').'/users/'.$item->id.'/'.$item->image) }}"/>
                                </div>
