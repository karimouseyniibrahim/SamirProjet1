...
@section('content')
    @component(layoutForm() , ['title' => trans('commandeproduit.commandeproduit') , 'model' => 'commandeproduit' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('admin/commandeproduit/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
		 ...
		 <div class="form-group {{ $errors->has("datedebutacheminement") ? "has-error" : "" }}" > 
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('commandeproduit.commandeproduit') }}
                </button>
            </div>
        </form>
    @endcomponent
